...
public class SwipeRightMonitor extends AbsSwipeMonitor {
    private static final String TAG = "SwipeRightMonitor";

    public SwipeRightMonitor(DavinciEventMonitor davinciEventMonitor) {
    ...
    public boolean isMovedTooFarAway(Event event) {
        ...
        return rawEventHistoryManager != null && Math.abs(rawEventHistoryManager.getPositionY()) > 0.3f && event.absDeltaX <= event.absDeltaY;
    }

    @Override // com.samsung.remotespen.core.device.control.detector.gesture.davinci.AbsSwipeMonitor
    public void onUpdateTravelDistance(GestureScore gestureScore, Event event) {
        if (event.deltaX > 0.0f) {
            gestureScore.increaseTravelDistance(event.absDeltaX);
        }
    }

    @Override // com.samsung.remotespen.core.device.control.detector.gesture.davinci.DavinciEventMonitor
    public BleSpenGestureType whoAmI() {
        return BleSpenGestureType.SWIPE_RIGHT;
    }
}
